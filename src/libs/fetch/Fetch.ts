...
export function myFetch(
  ...
  // temporary remove validate outdated format
  // .then(validResp);
  // }
  // return cachedData;
}
...
export const patchWithUrl = (url, params?, data?, options?, timeOut?: 10) => {
  return post(url, params, data, extend({ method: 'PATCH' }, options), timeOut);
};

export const deleteWithUrl = (url, params?, data?, options?, timeOut?: 10) => {
  return post(
    url,
    params,
    data,
